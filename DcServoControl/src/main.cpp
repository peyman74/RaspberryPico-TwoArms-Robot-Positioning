...
void loop() {
  ...
  if (zeroSwitchInstalled && zeroInterpolationDone) {
    ...
    for (int i = 0; i < numMotors; i++) {
      if (!motors[i]->getZeroSwitchPinState()) {  // Switch active = collision/emergency zone
        ...
        if (!zeroHandlingZoneDone) {
          DBGL("Motor ");
          DBG(i + 1);
          DBG(" collided with Emergency (here zero) swith. please pull the Motor out from emergency zone ");
        }
    ...
    if (anyCollision) {
      ...
      zeroInterpolationDone = false;  // Require re-zero interpolation after collision
